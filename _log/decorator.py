...
from .logger import scan_log, LogLevel
...
def logger(module_name:str, log_level:LogLevel=None):
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            # Logging arguments
            scan_log.log_result(log_level, module_name, "INP", f"Calling async {func.__name__} with args: {args} kwargs: {kwargs}")
            try:
                # Calling the function
                result = await func(*args, **kwargs)
                # Logging the result
                scan_log.log_result(log_level, module_name, "OUT", f"Async {func.__name__} returned {result}")
                return result
            except Exception as e:
                # Logging the error
                scan_log.log_result(log_level, module_name, "ERR", f"Error in async {func.__name__}: {e}")
                raise
